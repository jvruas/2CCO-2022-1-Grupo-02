...
public interface ProdutoRepository extends JpaRepository<ProdutoDoacao, Integer>{
	...
	@Query("select new com.conture.apiproduto.model.dto.response.ProdutoDoacaoHistoricoResponse('DOADO', p.categoriaProduto.nome, p.nome, p.dataCriacao, p.dataConclusao, p.fkDoador) from ProdutoDoacao p where p.fkDoador = ?1 and p.status = true and p.removido = false order by p.dataConclusao desc")
	List<ProdutoDoacaoHistoricoResponse> getAllByStatusDoado(Integer idDoador);
	...
	@Query("update ProdutoDoacao p set p.bucketName = ?2, p.objectName = ?3 where p.idProdutoDoacao = ?1 and p.removido = false and p.status = false")
	@Transactional
	@Modifying
	void updateProdutoImagemPrincipal(Integer idProdutoDoacao, String bucketName, String objectName);
}
