...
public interface AvaliacaoRepository extends JpaRepository<Avaliacao, Integer> {
	...
	@Query("select case when count(a) >= 1 then true else false end from Avaliacao a where a.match.produtoDoacao.fkDoador = ?1 and a.match.produtoDoacao.status = true and a.match.produtoDoacao.removido = false")
	...
	@Query("select new com.conture.apiproduto.model.dto.response.EstatisticasAvaliacaoResponse(avg(a.valor), count(a)) from Avaliacao a where a.match.produtoDoacao.fkDoador = ?1 and a.match.produtoDoacao.status = true and a.match.produtoDoacao.removido = false")
	EstatisticasAvaliacaoResponse getStatistcsByIdDoador(Integer idDoador);
}
